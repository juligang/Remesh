...
namespace dk
{
	...
	// constructor
	//
    MeshRetile::MeshRetile( const std::vector<math::Vec3d> &_vertices, const std::vector<int> &indicees )
	{
        for( std::vector<math::Vec3d>::const_iterator it=_vertices.begin(); it != _vertices.end(); ++it )
		{
			// create a vertex and
	...
	//
	//
    void MeshRetile::computeVertexIndicees( void )
	{
	...
	//
	//
    void MeshRetile::computeNormals( void )
    {
		// reset vertex normals
		for( std::vector<Vertex *>::iterator it = vertices.begin(); it != vertices.end(); ++it )
            (*it)->normal = math::Vec3d();

		// compute face normals
		for( std::vector<Triangle *>::iterator it = triangles.begin(); it != triangles.end(); ++it )
        {
			//compute face normal
            math::Vec3d faceNormal = math::normalize( math::crossProduct( (*it)->v1->position - (*it)->v0->position, (*it)->v2->position - (*it)->v0->position ) );
    ...
    void MeshRetile::assertManifold()
	{
...
//				printf( "degenerated vertex detected (referenced by less then 3 triangles) -> removed\n" );
                std::cout << "degenerated vertex detected (referenced by less then 3 triangles) -> removed" << std::endl;
    ...
    void MeshRetile::removeVertex( Vertex *v )
	{
    ...
    void MeshRetile::removeTriangle( Triangle *t )
	{
