...
namespace dk
{
    namespace io
    {
        ...
        void testObjFile()
        {
            std::cout<<"TEST OPENFILE" << std::endl;
        }
        //
        // reads and creates a MeshRetile from the given *.obj file. returns 0 if it fails
        //
        MeshRetile *importFromObjFile( std::string fileName )
        {
            ...
            if( !file )
            {
                std::cout<< "ERROR: Openinf the file" << fileName << std::endl;
            ...
            return new MeshRetile( vertices, indicees );
        }

        //
        // creates and writes a Obj file from the given mesh
        //
        void exportToObjFile( MeshRetile *mesh, std::string fileName )
        {
            ...
            for( std::vector<MeshRetile::Triangle *>::iterator it=mesh->triangles.begin(); it != mesh->triangles.end(); ++it, ++currentTriangleIndex )
            {
...
//            len = strftime ( time_buffer, 40, "%d %B %Y %I:%M:%S %p", &_tm );
        ...
        /*
            ...
            for( std::vector<math::Vec3d>::iterator it=mesh->normalBuffer.begin(); it != mesh->normalBuffer.end(); ++it )
