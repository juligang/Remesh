...
// constructor
//
RetileSurface::RetileSurface(int iterationCount)
{
    // initialize members
    m_radius        = 0.0;
    m_damp         = 0.15; // damping factor of the repulsion forces
    m_iterationCount = iterationCount;

    //Itearion many times the point was rotated around an edge
    m_iterationAroundEdge = 20;
...
void RetileSurface::generateVertices( size_t number )
{
    ...
    for( size_t i=0; i<mesh->m_triangles.size(); ++i )
    {
        ...
        area_sum += area;
//        std::cout << "Triangle["<< i << "] area = " << area << " area_sum = " << area_sum << std::endl;
        areas[area_sum] = mesh->m_triangles[i];
    }

    m_radius = 2.0*sqrt( area_sum / double(number)  );
    ...
    for( size_t i=0; i<number; ++i )
    {
        ...
        double b = (1.0 - a) * math::g_randomNumber();
        double c = 1.0 - a - b;
...
//	}while( ++count < 100 );
    }while( ++count < m_iterationAroundEdge ); //CHANGE FTHE NUMBER OF ITERATIONS
...
math::Vec3d RetileSurface::mapPointToPlane( const math::Vec3d &q, MeshEx::Triangle *t, MeshEx::Edge *hingeEdge )
{
    ...
    // rotation angle around edge
    double dihedralAngle = acosf( math::dotProduct( t->normal, adjNormal ) );

    double check = math::dotProduct( math::crossProduct(axis, t->normal) , math::normalize( t->center - edge->v1->position) );
    double check2 = math::dotProduct( t->normal , math::normalize( q - edge->v1->position) );
...
void RetileSurface::performIteration( double radius, double damp )
{
    ...
    math::Vec3d volMin(std::numeric_limits<double>::max(),
                       std::numeric_limits<double>::max(),
                       std::numeric_limits<double>::max());
    math::Vec3d volMax(std::numeric_limits<double>::min(),
                       std::numeric_limits<double>::min(),
                       std::numeric_limits<double>::min());
    for(std::vector<Vertex*>::const_iterator it = vertices.begin(); it != vertices.end(); ++it)
    {
        math::Vec3d curPos = (*it)->position;

        if (curPos.x < volMin.x) volMin.x = curPos.x;
        if (curPos.y < volMin.y) volMin.y = curPos.y;
        if (curPos.z < volMin.z) volMin.z = curPos.z;

        if (curPos.x > volMax.x) volMax.x = curPos.x;
        if (curPos.y > volMax.y) volMax.y = curPos.y;
        if (curPos.z > volMax.z) volMax.z = curPos.z;
    }
    volMin -= radius;
    volMax += radius;

    // Initialize the grid and insert the vertices
    SpatialGrid<int> grid(radius, volMin, volMax);
    for (int i = 0; i < vertices.size(); ++i)
        grid.insert(i, vertices[i]->position);
    ...
    for( size_t i=0; i<vertices.size(); ++i)
    {
        ...
        // reset force
        p->force = math::Vec3d( 0.0, 0.0, 0.0 );

        // Use the grid to get vertex IDs from the neighbouring area
        std::vector<int*> neighbourIds;
        grid.getElements(p->position, radius, neighbourIds);

        // determine nearby points
        std::vector<Vertex *>   neighbours;
        //for( size_t j=0; j<vertices.size(); ++j) // Old way -- checks against all vertices (j becomes **it)
        for (std::vector<int*>::const_iterator it = neighbourIds.begin(); it != neighbourIds.end(); ++it)
            if( i != **it )
                // Check if the vertex is within the radius and make sure
                // the angle between the normals is not greater than 90 degrees
                if( math::distance( p->position, vertices[**it]->position ) < radius
                        && math::dotProduct(p->t->normal, vertices[**it]->t->normal) >= 0.0 )
                    neighbours.push_back( vertices[**it] );
...
{
    std::cout << "1-RetileSurface::retile" << std::endl;
    mesh = _mesh;

    // clear the list of vertices
    clearVertices();

    std::cout << "2-RetileSurface::retile" << std::endl;
    ...
    std::cout << "5-RetileSurface::retile" << std::endl;
    //writeVertices( vertices, "retile_vertices_after.pcl" );
    //return;
    ...
    {
        std::cout << "5-.1 RetileSurface::retile Iterarion: " << i << std::endl;
        performIteration( m_radius, m_damp );
    }

    std::cout << "6-RetileSurface::retile" << std::endl;
    //writeVertices( vertices, "retile_vertices_after.pcl" );
    //return;
    ...
    std::cout << "7-RetileSurface::retile" << std::endl;
    // 2. remove old vertices
    std::vector<MeshEx::Vertex *> retainedVertices; // vertices which could not be removed

    printf( "retriangulating...\n" );

    std::cout << "8-RetileSurface::retile oldVertices.size() = " << oldVertices.size() << std::endl;
    for( size_t i=0; i<oldVertices.size(); ++i )
    {
        //std::cout << "8.1-RetileSurface::retile vetice = " << i << " of Tot vertices; "<< oldVertices.size()<<std::endl;
        bool removed = mesh->removeVertexAndReTriangulateNeighbourhood( oldVertices[i] );

//        std::cout << "8.2-RetileSurface::retile vetice = " << i << std::endl;
        // if vertex could not be removed
        if( !removed )
            // try again later...
            retainedVertices.push_back( oldVertices[i] );

//        std::cout << "8.3-RetileSurface::retile vetice = " << i << std::endl;
    }

    std::cout << "9-RetileSurface::retile" << std::endl;
    {
    ...
    std::cout << "10-RetileSurface::retile" << std::endl;
...
//	vsprintf_s( filename, 2048, filenameFormat, argumentList);
    snprintf( filename, 2048, filenameFormat, argumentList);
    va_end(argumentList);
