...
public:
	...
	struct Vertex
	{
        ...
        int               getPlaneSide( const math::Vec3d &normal, const double &distance );  // returns 1 if the node lies on the left side of the given plane (nodeplanedistance < 0.0) or 2 otherwise - returns 0 if point lies on the plane
	...
	struct Triangle
	{
        ...
        math::Matrix33d                                                                               beta; // barycentric coordinate matrix -> base vectors are the position of the 3 nodes of the element in the local 2d cooridnate frame
    ...
    MeshEx( MeshRetile *mesh );                                                                             ///< constructor which takes a mesh as input
	~MeshEx();                                                                                        ///< destructor
    MeshRetile                                                                                  *getMesh(); ///< creates a mesh which represents the MeshEx
...
//	void retriangulateHole( std::vector<MeshEx::Edge *> &boundaryEdges, std::map<MeshEx::Vertex *, math::Vec2d> &boundaryVertexProjections, std::vector<std::pair<math::Vec3d, math::Vec2d> > &interiorPoints);
	...
	struct VertexRingEdges
	{
        ...
        double computeClockwiseAngle( const math::Vec3d vec )
		{
            ...
            // which means the angle between those 2 vectors is either 0.0 or 180.0
			...
			*/
            ...
            if( math::dotProduct( m_normal, cp ) > 0.0 )
			...
			else
			{
				...
				// is 360 - angle
                //return math::degToRad( 360.0 ) - angle;
                //double t = math::degToRad( 360.0 ) - angle;
                double t = 2.0*M_PIl - angle;
