...
MeshEx::MeshEx( MeshRetile *mesh )
{
    ...
    for( std::vector<MeshRetile::Vertex *>::iterator it=mesh->vertices.begin(); it != mesh->vertices.end(); ++it )
	{
        MeshRetile::Vertex *vert = *it;
    ...
    for( std::vector<MeshRetile::Triangle *>::iterator it=mesh->triangles.begin(); it != mesh->triangles.end(); ++it )
	{
        MeshRetile::Triangle *tri = *it;
	...
	*/
    ...
    std::cout<<"CHECK FUNTION detectAndFillHoles" << std::endl;
    detectAndFillHoles();
}
...
//
MeshRetile *MeshEx::getMesh()
{
	std::map<Vertex *, int>        indicees; // the index into the vertexvector for each vertex
    std::vector<math::Vec3d> vertexPosition; // position for each vertex
    ...
    return new MeshRetile( vertexPosition, triangleIndicees );
}
...
//
MeshEx::Vertex *MeshEx::splitEdge( Edge *edge, const math::Vec3d &splitPosition )
{
...
//
MeshEx::Vertex *MeshEx::createVertex( const math::Vec3d &position )
{
	Vertex *n = new Vertex();
	n->position = position;
...
{
    static int edgesNotFoundCounter = 0;
	Triangle *t = new Triangle();
	...
	e0 = e1 = e2 = 0;
//    std::cout << "**8.1.19.1.1-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;
...
//    std::cout << "**8.1.19.1.2-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;

    if (e0 == NULL || e1 == NULL || e2 == NULL)
    {
        std::cerr << "Error trying to create triangle: could not find all the edges!\n\tv0 is ("
                     << v0->position.x << ", " << v0->position.y << ", " << v0->position.z << ")"
                        << " [total count: " << ++edgesNotFoundCounter << "]" << std::endl;
        delete t;
        return NULL;
    }
...
//    std::cout << "**8.1.19.1.3-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;
	for( int i=0; i<3; ++i )
    {
//        std::cout << "**8.1.19.1.3-MeshEx::removeVertexAndReTriangulateNeighbourhood Iteration: "<< i<<std::endl;
		t->e[i]->registerTriangle( t );
    }

	t->computeProperties();

//    std::cout << "**8.1.19.1.4-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;
...
struct helperTemp
{
    helperTemp( MeshEx::Vertex *_v1, MeshEx::Vertex *_v2 ) : v1(_v1), v2(_v2) { sqDistance = (v2->position - v1->position).getSquaredLength(); }
    double sqDistance;
    MeshEx::Vertex *v1;
    MeshEx::Vertex *v2;

    bool operator<( const helperTemp &rhs ) const
    {
...
//
void MeshEx::reTriangulate( Triangle *t, const std::vector<math::Vec3d> &pointsWithin )
{
    // prepare -------------------------------------------------------------
    ...
    for( std::vector<math::Vec3d>::const_iterator it=pointsWithin.begin(); it != pointsWithin.end(); ++it )
	{
    ...
    std::vector<helperTemp> sqDistances;  // sorted distances of each pair of points

	for( size_t i=0; i<vertices.size()-1; ++i )
		for( size_t j=i+1; j<vertices.size(); ++j )
            sqDistances.push_back( helperTemp( vertices[i], vertices[j] ) );
	...
	for( size_t i=0; i<3; ++i )
		for( size_t j=0; j<vertices.size(); ++j )
            sqDistances.push_back( helperTemp( v[i], vertices[j] ) );
    ...
    for( std::vector<helperTemp>::iterator it=sqDistances.begin(); it != sqDistances.end(); ++it )
	{
        helperTemp *h = &(*it);
...
//
//
void MeshEx::createTrianglesFromEdges( std::vector<Edge *> &edges, const math::Vec3d &normal )
{
	...
	for( std::vector<Vertex *>::iterator it = vertices.begin(); it != vertices.end(); ++it)
	{
        ...
        math::Vec3d vec_edge1;
        math::Vec3d vec_edge2;
        math::Vec3d vec_edge3;
		...
		for( std::vector<Edge *>::iterator eit = v1_edgeList.begin(); eit != v1_edgeList.end(); ++eit )
		{
			...
			for( std::vector<Edge *>::iterator eit2 = v2_edgeList.begin(); eit2 != v2_edgeList.end(); ++eit2 )
			{
                ...
                math::Vec3d vec_edge2c = math::normalize( ec2->getOtherVertex(v2)->position - v2->position );
                ...
                double angle = math::dotProduct( vec_edge1, -vec_edge2c );
                ...
                if( (e2_side<0.0) && ((angle > minAngle) || !minEdge) )
				{
					minEdge = ec2;
					minAngle = angle;
			...
			for( std::vector<Edge *>::iterator eit3 = v3_edgeList.begin(); eit3 != v3_edgeList.end(); ++eit3 )
			{
                ...
                double angle = math::dotProduct( vec_edge2, -vec_edge3 );

				// by restricting e2_direction to be positive, we only look at edges which would build
				// the triangle in the right order
                if( (e3_side<0.0) && ((angle > minAngle) || !minEdge) )
				{
					minEdge = ec3;
					minAngle = angle;
				}
...
void MeshEx::Triangle::computeProperties()
{
    ...
    math::Vec3d u_unormalized = v[1]->position - v[0]->position;
    math::Vec3d         n3vec = v[2]->position - v[0]->position;
    ...
    double la = (v1->position - v0->position).getLength(); // compute lengths of the triangle sides
    double lb = (v2->position - v1->position).getLength();
    double lc = (v2->position - v0->position).getLength();
...
//
// Returns 1 if the node lies on the left side of the given plane (nodeplanedistance < 0.0)
// or 2 otherwise.
...
int MeshEx::Vertex::getPlaneSide( const math::Vec3d &normal, const double &distance )
{
     ...
     // if the planeDistance is 0.0 then the point lies directly on the plane and can not be
...
//    return math::intersectionRayPlane( math::Ray3f( v1->position, v2->position ), normal, distance, intersectionPoint );
}
...
//    return math::intersectionRayRay( math::Ray3f( v1->position, v2->position ), math::Ray3f( p1, p2 ), intersectionPoint );
}
...
//    return math::intersectionRayRay( math::Ray3f( v1->position, v2->position ), math::Ray3f( p1, p2 ), intersection );
}
...
{
//    std::cout << "**8.1.19.1.3.0-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;
	if( !left )
	{
//        std::cout << "**8.1.19.1.3.1-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;
		left = t;
	}else
	if( !right )
	{
//        std::cout << "**8.1.19.1.3.2-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;
		right = t;
	...
	{
//        std::cout << "**8.1.19.1.3.3-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;
		printf( "error - edge must not belong to more than 2 m_triangles\n" );
//        std::cout << "*********8.1.19.1.3.31-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;
		tag = true;
//        std::cout << "*********8.1.19.1.3.32-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;
		left->tag = true;
//        std::cout << "*********8.1.19.1.3.33-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;

		left->tag2 = true;
//        std::cout << "*********8.1.19.1.3.34-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;
	}
//    std::cout << "**8.1.19.1.3.4-MeshEx::removeVertexAndReTriangulateNeighbourhood "<<std::endl;
}
